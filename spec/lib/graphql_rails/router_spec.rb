...
module GraphqlRails
  RSpec.describe Router do
    ...
    describe '#graphql_schema' do
      ...
      context 'when subscribtion type is set' do
        before do
          test_subscription = Class.new(GraphQL::Schema::Object) do
            field :test_subscription, subscription: GraphQL::Schema::Subscription
          end

          stub_const('SubscriptionType', test_subscription)
        end

        let(:router) do
          described_class.draw do
            scope module: :graphql_rails do
              subscriptions_type SubscriptionType
            end
          end
        end

        it 'returns schema with subscription type' do # rubocop:disable RSpec/ExampleLength
          expect(graphql_schema.to_definition).to eq(
            <<~GRAPHQL
              type Query

              type Subscription {
                testSubscription: SubscriptionPayload!
              }
